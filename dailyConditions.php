...
<script language="javascript" type="text/javascript" src="rdloggerDaily.js"></script><script type="text/javascript" src="http://ian.aprsworld.com/data/jQueryRotate.2.2.js"></script>';
...
$sql=sprintf("SELECT DAYOFYEAR(DATE_SUB(packet_date,INTERVAL 5 HOUR)) AS dayOfYear, YEAR(DATE_SUB(packet_date,INTERVAL 5 HOUR)) AS year, LEFT(DATE_SUB(packet_date,INTERVAL 5 HOUR),10) AS day, UNIX_TIMESTAMP(LEFT(DATE_SUB(packet_date,INTERVAL 5 HOUR),10)) AS x, MAX(windGust) AS pulseGust0Max, ROUND(AVG(windSpeed),2) AS pulseCurrent0Average FROM rdLoggerCell_%s GROUP BY DAYOFYEAR(DATE_SUB(packet_date,INTERVAL 5 HOUR)) ORDER BY YEAR(DATE_SUB(packet_date,INTERVAL 5 HOUR)) DESC, DAYOFYEAR(DATE_SUB(packet_date,INTERVAL 5 HOUR)) DESC",$station_id);
//echo $sql;
