...
$sql=sprintf("SELECT LEFT(DATE_ADD(packet_date,INTERVAL %d HOUR),13) AS date_hour, ROUND(AVG(windSpeed),2) AS windAverage, MAX(windGust) AS windGust, COUNT(*) AS nPackets FROM rdLoggerCell_%s WHERE LEFT(DATE_ADD(packet_date,INTERVAL %d HOUR),10)='%s' GROUP BY date_hour",$tzOffset,$station_id,$tzOffset,$day);
...
/*
	if ( $r["windCount"] > 0 ) 
		...
		printf("<td>0.0</td>");
*/
	printf("<td>%d%%  (%d total)</td>",($r["nPackets"]/3.6),$r["nPackets"]);
	printf("</tr>\n");
}
